...
class ConCapModel(BlipPreTrainedModel):
    ...
    def forward(self, 
        ...
        dec_logits=text_lm_output.logits[:, pfx_len:, :]
        loss_lm = None

        if labels is not None:
            # mask fill
            labels=labels.masked_fill(labels==self.pad_token_id, -100)
            # next-word prediction CE loss
            shift_logits=dec_logits[:, :-1, :].contiguous()
            labels=labels[:,1:].contiguous().to(shift_logits.device)
            loss_lm=torch.nn.CrossEntropyLoss(label_smoothing=0.1)(
